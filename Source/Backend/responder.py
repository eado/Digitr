...
class Responder:
        ...
        for email in users:
            ...
            if {'user': email, 'message': message['timestamp']} not in nots_sent:
                for noti in user.get('notifications'):
                    if noti['type'] == 'ios':
                        apns = APNs(use_sandbox=False, cert_file="/var/server/Digitr3/Source/Backend/cert-live.pem", key_file='/var/server/Digitr3/Source/Backend/key-live.pem')
    ...
    def get_stats(self, use_admin):
        ...
        if not use_admin and not self.request.get('all'):
    ...
    def start_payment(self):
        ...
        if district.get('last_payment_count', 0) >= district.get('max_count', 0) or not district.get('last_payment_count'):
